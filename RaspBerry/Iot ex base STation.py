...
def on_message(client, userdata, msg):
    ...
    for result in results:
       print(result)
    query = "INSERT INTO expenses_expense (amount,description,date) VALUES (%s, %s, %s)"
    paras = (waterlevel,fire,datetime.datetime.now())
...
from urllib.parse import urlencode
 ...
 data1= {"waterlevel":waterlevel,"fire":fire}
    http= urllib3.PoolManager()
    encoded_data=urlencode(data1)
    http.request('POST','https://station01.000webhostapp.com/sleam.php?'+encoded_data,headers={'Content-Type': 'application/json'})
